...
create or replace function public.knowledge_sync_get_checkpoint(
...
as $$
  ...
  from (select 1) seed
  left join knowledge.knowledge_sync_state s
    on s.connector_key = p_connector_key;
$$;

create or replace function public.knowledge_sync_synthetic_fixture_status(
...
as $$
  select case when s.id is null then jsonb_build_object('exists', false) else jsonb_build_object(
    ...
    'unit_count', (
      select count(*)
      from knowledge.knowledge_units u
...
comment on function public.knowledge_sync_get_checkpoint(text) is
  'Service-role-only Drive cursor read for the controlled server-side sync runner.';
comment on function public.knowledge_sync_synthetic_fixture_status(text) is
  'Service-role-only metadata/count status for rows explicitly marked as synthetic M2B fixtures. Returns no document text.';
