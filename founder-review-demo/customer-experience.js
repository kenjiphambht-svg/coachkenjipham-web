...
(function () {
  ...
  window.EssenceCustomerExperience = function createCustomerExperience(ctx) {
    ...
    function renderTemplate() {
      const state=cx(); const template=templates[state.template] || templates.hatmam; const chapter=Math.min(state.chapter,template.sections.length);
      ...
      return customerPage(template.label, template.tone, `
        ...
        ${autonomy}
        <section class="customer-grid"><aside class="customer-card customer-toc"><p class="customer-kicker">MỤC LỤC</p>${template.sections.map((s,i)=>`<button class="customer-chapter ${chapter===i+1?'current':''}" data-action="cx-chapter-${i+1}">${i+1}. ${s}</button>`).join('')}</aside><article class="customer-reading"><p class="customer-kicker">PHẦN ${chapter}/${template.sections.length}</p><h2>${template.sections[chapter-1]}</h2><p>Đây là nội dung đọc mẫu để Founder đánh giá nhịp, khoảng trắng và cách dẫn dắt. Không có văn bản của khách, trẻ em hay ấn phẩm thật.</p><blockquote>“Đọc chậm cũng là một cách để giữ lại điều vừa chạm tới.”</blockquote><p class="small">Mô phỏng tiến độ chỉ nằm trên thiết bị này.</p><div class="button-set">${chapter>1?ctx.action('Quay lại','cx-prev'):''}${chapter<template.sections.length?ctx.action('Tiếp theo','cx-next'):''}</div></article></section>
        <section class="customer-grid"><article class="customer-card"><h2>Đọc và giữ riêng</h2><p>PDF A5 chỉ là nút minh họa. Bản thật cần entitlement, Private Storage, checksum và signed URL ngắn hạn.</p>${ctx.action('Tải PDF A5 (mô phỏng)','cx-pdf')}</article><article class="customer-card"><h2>Cần hỗ trợ?</h2><p>Link, hiển thị, clarification, revision khi contract cho phép hoặc deletion concern.</p>${ctx.action('Gửi yêu cầu hỗ trợ giả','cx-support')}</article></section>
        <section class="customer-door"><p class="customer-kicker">CÁNH CỬA TIẾP THEO</p><h2>Có thể phù hợp với nơi bạn đang đứng</h2><p>Tối đa một đề xuất, không checkout, không khan hiếm và không suy luận từ nội dung riêng tư.</p>${ctx.action('Xem Bản Sắc Khám Phá (mô phỏng)','cx-next-khampha')}</section>
    ...
    function handleAction(id) {
      ...
      if(id==='cx-preview-template'){ctx.notice('Preview responsive là mô phỏng; không có route, provider hoặc public activation.');return true;}
