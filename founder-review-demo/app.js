...
(function () {
  ...
  const initial = {
    ...
    settings: { price: 2000000, capacity: 10, delivery: 5, revision: 7, retention: 12 },
    versions: [{ number: 3, time: '02/08/2026, 16:10', label: 'Phiên bản đang dùng', price: 2000000, capacity: 10 }],
    wizard: { start: null, step: 1, saved: false },
    assistantOpen: false,
    audit: [
  ...
  const nav = [
    ['overview', 'Tổng quan'], ['growth', 'Khách hàng'], ['work', 'Công việc'],
    ['catalog', 'Cấu hình kinh doanh'], ['system', 'Hệ thống']
  ];
  ...
  function page(title, lede, body) { return `<p class="small">Founder Operating Console · ${esc(title)}</p><h1 class="view-title">${title}</h1><p class="lede">${lede}</p>${body}`; }
  ...
  function renderPayments() {
    return page('Thanh toán', 'Kiểm tra biên nhận demo — biên nhận giả chỉ gắn với đúng đơn giả HM-018.', `
      ...
      <section class="grid two"><article class="card"><h2>Biên nhận giả · HM-018</h2><div class="kv"><div class="muted">Package snapshot</div><div>HM-01 · Ấn phẩm Bản Sắc — phiên bản 3</div><div class="muted">Amount snapshot</div><div>₫2.000.000</div><div class="muted">Transfer reference</div><div>DEMO-HM-018</div><div class="muted">Evidence synthetic</div><div>${badge('ĐÃ ĐÍNH KÈM','ready')}</div><div class="muted">Reported transfer</div><div>03/08/2026, 09:35</div></div><div class="button-set">${action(data.payment.toLowerCase().includes('xác nhận') ? 'Đã xác nhận' : 'Xác nhận payment giả','confirm-payment', data.payment.toLowerCase().includes('xác nhận') ? 'button-quiet' : '')}</div></article>
  ...
  function renderHatmam() {
    return page('Hạt Mầm', 'Danh sách đơn dữ liệu giả. Giá và thời hạn của từng đơn được bảo toàn từ package snapshot.', `
      <div class="table-wrap"><table class="table"><thead><tr><th>Mã đơn</th><th>Trạng thái</th><th>Giao trước</th><th>Chỉnh sửa đến</th><th></th></tr></thead><tbody>
        <tr><td><strong>HM-018</strong><br><span class="small">HM-01 · Snapshot v3 · ₫2.000.000</span></td><td>${badge(data.hatmamStatus,statusTone(data.hatmamStatus))}</td><td>08/08/2026</td><td>15/08/2026</td><td>${link('Mở đơn','hatmam-detail')}</td></tr>
        <tr><td><strong>HM-013</strong><br><span class="small">HM-02 · Snapshot v2 · ₫3.500.000</span></td><td>${badge(data.publication,'pending')}</td><td>05/08/2026</td><td>12/08/2026</td><td>${link('Review','publication')}</td></tr>
        <tr><td><strong>HM-011</strong><br><span class="small">HM-01 · Snapshot v2 · ₫2.000.000</span></td><td>${badge('Đang sản xuất','pending')}</td><td>02/08/2026</td><td>09/08/2026</td><td>${badge('QUÁ HẠN','blocked')}</td></tr>
      </tbody></table></div>
  ...
  function renderHatmamDetail() {
    return page('Đơn Hạt Mầm · HM-018', 'Một đơn giả để Founder thử toàn bộ state transition theo thứ tự.', `
      ...
      <section class="card" style="margin-top:14px"><h2>Snapshot integrity</h2><div class="kv"><div class="muted">Tên package đã snapshot</div><div>HM-01 · Ấn phẩm Bản Sắc</div><div class="muted">Giá đã snapshot</div><div>₫2.000.000</div><div class="muted">Delivery snapshot</div><div>5 business days</div><div class="muted">Retention snapshot</div><div>Raw intake 12 tháng · private room 24 tháng</div></div></section>`);
  }
  ...
  const productRows = [
    ['Ebook & Góc đọc','Khám phá miễn phí','Bản nháp','Email/route chưa kết nối','Asset, consent, reading route, provider','Hoàn thiện contract'],
    ['Tôi đang ở đâu?','Người lớn','Bản nháp · 50.000 VND','Rule deterministic v0','Câu hỏi, rule, kết quả, email Founder duyệt','Duyệt contract assessment'],
    ['Lặng 90’','Người lớn','Kiến trúc canonical','Provider thực OFF','Auth/AAL2, payment, Cal.com, real E2E','Đóng release gates'],
    ['Bạn Là Duy Nhất','Người lớn','Xem trước','Ấn phẩm riêng tư chưa kết nối','Form, giá, SLA, revision, policy','Duyệt product contract'],
    ['Dấu Ấn Của Bạn','Người lớn','Xem trước','Session 150 phút + ấn phẩm','Giá, form, refund, booking, fulfilment','Duyệt product contract'],
    ['Hạt Mầm · HM-01','Phụ huynh / 0–7','Canonical · public blocked','2m launch / 3m reference','Storage, deletion, real E2E','Giữ gates OFF'],
    ['Hạt Mầm · HM-02','Phụ huynh / 0–7','Canonical · public blocked','3.5m launch / 5.5m reference','Storage, deletion, real E2E','Giữ gates OFF'],
    ['Bản Sắc Khám Phá','7–14','Xem trước','Không kế thừa Hạt Mầm','Giá, form, consent, workflow riêng','Duyệt contract riêng'],
    ['Bản Sắc Giao Mùa','14–21','Xem trước','Autonomy boundary mở','Participation, access, change, deletion','Founder decision riêng'],
    ['KIDMAP / payments cũ','Legacy','Legacy','Cô lập','Không cấu hình/migrate/reuse','Không tác động']
  ];
  function productTable() { return `<div class="table-wrap"><table class="table"><thead><tr><th>Sản phẩm</th><th>Đối tượng</th><th>Trạng thái</th><th>Giá / workflow</th><th>Thiếu / gate</th><th>Bước được phép</th></tr></thead><tbody>${productRows.map(r=>`<tr>${r.map((c,i)=>`<td>${i===2?badge(c,statusTone(c)):esc(c)}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`; }

  function renderWork() {
    return page('Công việc', 'Các hàng chờ theo việc Kenji cần làm. Chỉ Lặng và Hạt Mầm có fixture thao tác; mọi ấn phẩm khác là preview contract.', `<section class="grid two"><article class="card"><h2>Lặng</h2><p>${badge('CANONICAL · EXTERNAL GATES OPEN','pending')}</p><p class="small">Intake 6 câu → Human Decision Gate → payment → booking riêng. Không AI quyết định, không booking trước payment.</p><div class="button-set">${action('Mở Lặng','go-lang')} ${action('Mở payment','go-payment')}</div></article><article class="card"><h2>Ấn phẩm người lớn</h2><p>${badge('2 PREVIEW CONTRACTS','pending')}</p><p class="small">Bạn Là Duy Nhất và Dấu Ấn Của Bạn không có route, form hay delivery thật.</p><div class="button-set">${action('Xem kiến trúc','go-adult-publications')}</div></article><article class="card"><h2>Bản Sắc Của Con</h2><p>${badge('HM CANONICAL · PUBLIC BLOCKED','blocked')}</p><p class="small">HM-01/HM-02 tách package snapshot. Khám Phá/Giao Mùa là preview độc lập.</p><div class="button-set">${action('Mở Hạt Mầm','go-hatmam')} ${action('Xem preview line','go-child-preview')}</div></article><article class="card"><h2>Dữ liệu, payment & publication</h2><p class="small">Evidence và các thao tác destructive đều chỉ là fixture fail-closed.</p><div class="button-set">${action('Payment','go-payment')} ${action('Dữ liệu & ấn phẩm','go-data-publication')}</div></article></section>`);
  }

  function renderAdultPublications() {
    return page('Ấn phẩm người lớn', 'Hai contract preview để review cấu trúc trước khi có Founder approval sản phẩm riêng.', `<section class="grid two"><article class="card"><h2>Bạn Là Duy Nhất</h2>${badge('XEM TRƯỚC','pending')}<ol class="timeline"><li><time>1</time>Intake (chưa duyệt)</li><li><time>2</time>Payment request → Kenji xác nhận thật</li><li><time>3</time>Sản xuất → Kenji review → revision nếu contract cho phép</li><li><time>4</time>Private reading room + PDF (Storage OFF)</li></ol><p class="small">Thiếu: câu hỏi, chapter template, SLA, revision, giá và chính sách.</p></article><article class="card"><h2>Dấu Ấn Của Bạn</h2>${badge('XEM TRƯỚC','pending')}<ol class="timeline"><li><time>1</time>Intake / Kenji review nếu được duyệt</li><li><time>2</time>Payment → Kenji xác nhận thật</li><li><time>3</time>Private booking → session 150 phút</li><li><time>4</time>Ấn phẩm hai lớp → review → delivery riêng</li></ol><p class="small">Thiếu: giá, form, refund, booking và fulfilment terms.</p></article></section><div class="button-set">${action('Quay lại Công việc','go-work')} ${action('Xem catalog','go-catalog')}</div>`);
  }

  function renderChildPreview() {
    return page('Bản Sắc Khám Phá & Giao Mùa', 'Không dùng Hạt Mầm làm mặc định. Mỗi sản phẩm cần contract, consent và activation riêng.', `<section class="grid two"><article class="card"><h2>Khám Phá · 7–14</h2>${badge('XEM TRƯỚC','pending')}<p>Có thể dùng blocks đã duyệt, nhưng giá, form, consent và logic kết quả không tự kế thừa Hạt Mầm.</p><p class="small">Next allowed: Founder duyệt product-specific contract.</p></article><article class="card"><h2>Giao Mùa · 14–21</h2>${badge('XEM TRƯỚC · AUTONOMY OPEN','blocked')}<p>Phải quyết định awareness/participation của người trẻ, quyền truy cập ấn phẩm, sửa đổi và deletion trước implementation.</p><p class="small">Next allowed: Founder decision về autonomy boundary.</p></article></section><div class="button-set">${action('Quay lại Công việc','go-work')} ${action('Mở workflow library','go-workflows')}</div>`);
  }

  function renderCatalog() {
    return page('Sản phẩm & quy trình', 'Catalog trung tâm: mỗi sản phẩm có state độc lập; có trang không đồng nghĩa được bán hoặc active.', `<div class="notice"><strong>State model:</strong> Bản nháp → Xem trước → Sẵn sàng nội bộ → Đã kiểm thử → Chờ Founder mở → Đang hoạt động; có thể Tạm dừng, Ngừng nhận mới, Lưu trữ hoặc Legacy. Indexing luôn là quyết định M6 riêng.</div><div class="button-set" style="margin:14px 0">${action('Thêm sản phẩm giả','go-wizard')} ${action('Workflow library','go-workflows')} ${action('Giá & policy version','go-business-config')} ${action('Release checklists','go-product-readiness')}</div>${productTable()}`);
  }

  function renderWorkflows() {
    const blocks=['Khám phá / email capture / consent','Lead delivery / nurture','Intake / assessment / deterministic scoring','Kenji Decision Gate / yêu cầu thêm thông tin','Payment request / report / evidence / xác nhận','Booking / session','Sản xuất / review / revision / final approval','Private publication / email delivery / follow-up','Retention / deletion / audit'];
    return page('Workflow library', 'Founder chỉ có thể ghép block đã được duyệt sau khi backend thật tồn tại. Thiếu block là software/governance change.', `<section class="card"><h2>Block đã được chuẩn bị</h2><ul class="timeline">${blocks.map((b,i)=>`<li><time>${i+1}</time>${b}</li>`).join('')}</ul></section><section class="card" style="margin-top:14px"><h2>Ví dụ binding</h2><p class="small">Ebook: capture → operational consent → delivery → optional nurture. Assessment: versioned questions → result locked → payment evidence → deterministic result → email. Lặng: intake → Kenji decision → payment → booking. Hạt Mầm: intake/consent → snapshot → payment → production/review → private delivery → retention/deletion.</p></section><div class="button-set">${action('Quay lại catalog','go-catalog')} ${action('Thêm sản phẩm giả','go-wizard')}</div>`);
  }

  function renderBusinessConfig() {
    return page('Giá, chính sách & cấu hình', 'So sánh phiên bản chỉ minh họa. Snapshot của order cũ không bao giờ bị ghi đè bởi cài đặt mới.', `<section class="grid two"><article class="card"><h2>Phiên bản package Hạt Mầm</h2><div class="kv"><div class="muted">HM-01 launch / reference</div><div>2.000.000 / 3.000.000 VND</div><div class="muted">HM-02 launch / reference</div><div>3.500.000 / 5.500.000 VND</div><div class="muted">SLA / revision</div><div>5 ngày làm việc / 7 ngày</div><div class="muted">Retention</div><div>Raw intake 12 tháng / publication 24 tháng</div></div><p class="small">Áp dụng tương lai sau real implementation, không tự activate public flow.</p></article><article class="card"><h2>Configuration hay software?</h2><p>${badge('FOUNDER CONFIGURATION','ready')}</p><p class="small">Tên, label, giá/reference, effective date, capacity, SLA, policy copy, câu hỏi/email đã duyệt, block đã có, release request.</p><p>${badge('SOFTWARE / GOVERNANCE','blocked')}</p><p class="small">Capability/state/scoring/provider/payment automation/data category/access/deletion/AI/public activation/indexing mới → Draft PR, tests và Founder approval.</p></article></section><div class="button-set">${action('Cài đặt phiên bản demo','go-settings')} ${action('Quay lại catalog','go-catalog')}</div>`);
  }

  function renderProductReadiness() {
    return page('Release checklist theo sản phẩm', 'Không product nào được gắn “sẵn sàng” khi provider hoặc real E2E chưa được xác minh.', `<section class="table-wrap"><table class="table"><thead><tr><th>Sản phẩm</th><th>Contract / copy / form</th><th>Payment / provider</th><th>Privacy / delivery</th><th>Trạng thái honest</th></tr></thead><tbody><tr><td>Ebook</td><td>OPEN</td><td>Resend OFF</td><td>Consent/route OPEN</td><td>${badge('BẢN NHÁP','blocked')}</td></tr><tr><td>Assessment 50k</td><td>Question/rule/result OPEN</td><td>Manual payment contract OPEN</td><td>Real E2E/email OFF</td><td>${badge('BẢN NHÁP','blocked')}</td></tr><tr><td>Lặng</td><td>Canonical</td><td>Cal.com/payment E2E OPEN</td><td>Auth/AAL2 real evidence OPEN</td><td>${badge('KHÔNG ACTIVE','blocked')}</td></tr><tr><td>Hạt Mầm</td><td>Canonical HM-01/HM-02</td><td>Payment E2E OPEN</td><td>Storage/deletion OPEN</td><td>${badge('PUBLIC BLOCKED','blocked')}</td></tr><tr><td>Preview contracts</td><td>Founder inputs OPEN</td><td>OFF</td><td>OFF</td><td>${badge('XEM TRƯỚC','pending')}</td></tr></tbody></table></section><div class="callout fail" style="margin-top:14px">“Chờ Founder mở” chỉ có sau khi mọi checklist riêng, real provider E2E, monitoring, backup/rollback và feature flag OFF review hoàn tất. Product activation không thay indexing M6.</div><div class="button-set">${action('Quay lại catalog','go-catalog')}</div>`);
  }

  function renderWizard() {
    const starts=['Tạo từ mẫu có sẵn','Sao chép cấu trúc của một sản phẩm','Bắt đầu bản nháp trống']; const steps=['Thông tin cơ bản','Đối tượng','Gói và giá','Biểu mẫu','Workflow','Thanh toán','Giao sản phẩm','Chính sách','Email','Sức chứa','Dữ liệu và retention','Review','Release checklist']; const w=data.wizard;
    return page('Thêm sản phẩm — synthetic wizard', 'Wizard chỉ lưu một bản nháp trong localStorage. Nó không tạo route, table, form, email, payment, public product hay active offer.', `<section class="grid two"><article class="card"><h2>Bắt đầu</h2>${starts.map((s,i)=>`<p>${action(s,`wizard-start-${i}`,'button-secondary')} ${w.start===i?badge('ĐÃ CHỌN','ready'):''}</p>`).join('')}<p class="small">Mẫu có thể: ebook, paid assessment, private session, personalised publication, session + publication, parent/child publication.</p></article><article class="card"><h2>Bước ${w.step}/13 · ${steps[w.step-1]}</h2><p class="small">${w.start===null?'Chọn một cách bắt đầu trước.':'Nội dung bước này là khung review; real implementation cần contract và gates riêng.'}</p><div class="button-set">${w.step>1?action('Quay lại','wizard-back'):''}${w.step<13?action('Tiếp theo','wizard-next'):action('Lưu bản nháp local','wizard-save')}</div>${w.saved?`<p>${badge('BẢN NHÁP LOCAL ĐÃ LƯU','ready')}</p>`:''}</article></section><section class="card" style="margin-top:14px"><h2>Release boundary</h2><p class="small">Save không kích hoạt gì. “Đang hoạt động” không thể được tạo bằng wizard; route, provider, consent, payment, privacy, real E2E và Founder activation đều là gates tách riêng.</p></section><div class="button-set">${action('Quay lại catalog','go-catalog')}</div>`);
  }
  ...
  const renderers = { overview:renderOverview, growth:renderGrowth, work:renderWork, catalog:renderCatalog, lang:renderLang, 'lang-detail':renderLangDetail, payments:renderPayments, hatmam:renderHatmam, 'hatmam-detail':renderHatmamDetail, production:renderProduction, publication:renderPublication, deletion:renderDeletion, settings:renderSettings, audit:renderAudit, readiness:renderReadiness, contact:renderContact, 'data-publication':renderDataPublication, system:renderSystem, ebook:renderEbook, assessment:renderAssessment, nurture:renderNurture, 'adult-publications':renderAdultPublications, 'child-preview':renderChildPreview, workflows:renderWorkflows, 'business-config':renderBusinessConfig, 'product-readiness':renderProductReadiness, wizard:renderWizard };
  function navView() { return ({'lang':'work','lang-detail':'work','hatmam':'work','hatmam-detail':'work',production:'work',payments:'work',publication:'work',deletion:'work','data-publication':'work','adult-publications':'work','child-preview':'work',audit:'system',settings:'catalog',readiness:'system',contact:'system',ebook:'growth',assessment:'growth',nurture:'growth',workflows:'catalog','business-config':'catalog','product-readiness':'catalog',wizard:'catalog'})[data.view] || data.view; }
  ...
  function renderAssistant() { assistantPanel.innerHTML = data.assistantOpen ? `<section class="assistant-card"><h2>Hỏi Trợ lý</h2><p>Tóm tắt theo quy tắc trên dữ liệu giả — chưa kết nối AI thật.</p><ul><li>L-024 đang quá SLA và cần Kenji quyết định.</li><li>HM-018 có biên nhận đang chờ kiểm tra.</li><li>HM-011 quá hạn 1 ngày.</li><li>Lưu trữ riêng, email, lịch và kích hoạt công khai đều đang OFF.</li></ul><p class="small"><strong>Được phép trong tương lai:</strong> giải thích, tìm, tóm tắt, dự thảo và so sánh thay đổi. <strong>Không được:</strong> activate/publish/index, xác nhận payment, quyết định Lặng, score assessment, duyệt ấn phẩm, xóa dữ liệu, đổi Auth/MFA/quyền, kết nối provider hoặc đổi giá không có Founder confirmation. Mọi đề xuất phải có before/after, ảnh hưởng order tương lai/lịch sử và effective date.</p><div class="button-set">${action('Đóng','close-assistant')}</div></section>` : ''; }
  ...
  function handleAction(id) {
    const updates = {
      'go-lang':()=>go('lang-detail'),'go-work':()=>go('work'),'go-catalog':()=>go('catalog'),'go-payment':()=>go('payments'),'go-deletion':()=>go('deletion'),'go-settings':()=>go('settings'),'go-hatmam':()=>go('hatmam'),'go-hatmam-detail':()=>go('hatmam-detail'),'go-publication':()=>go('publication'),'go-audit':()=>go('audit'),'go-system':()=>go('system'),'go-readiness':()=>go('readiness'),'go-contact':()=>go('contact'),'go-growth':()=>go('growth'),'go-ebook':()=>go('ebook'),'go-assessment':()=>go('assessment'),'go-nurture':()=>go('nurture'),'go-adult-publications':()=>go('adult-publications'),'go-child-preview':()=>go('child-preview'),'go-data-publication':()=>go('data-publication'),'go-workflows':()=>go('workflows'),'go-business-config':()=>go('business-config'),'go-product-readiness':()=>go('product-readiness'),'go-wizard':()=>go('wizard'),
      ...
      ,'wizard-back':()=>{data.wizard.step=Math.max(1,data.wizard.step-1);data.wizard.saved=false;}
      ,'wizard-next':()=>{if(data.wizard.start===null){notice('Chọn một cách bắt đầu trước.');return;}data.wizard.step=Math.min(13,data.wizard.step+1);data.wizard.saved=false;}
      ,'wizard-save':()=>{if(data.wizard.start===null){notice('Chọn một cách bắt đầu trước.');return;}data.wizard.saved=true;audit('Đã lưu một product draft synthetic trong localStorage; không tạo route, API hoặc public offer.');notice('Đã lưu bản nháp local. Không có sản phẩm thật được tạo hoặc kích hoạt.');}
    };
  ...
  const fixtures = [
    ...
    { text:'Tôi đang ở đâu? · assessment 50.000 VND', view:'assessment' },
    { text:'Bạn Là Duy Nhất · preview', view:'adult-publications' },
    { text:'Bản Sắc Giao Mùa · autonomy boundary', view:'child-preview' },
    { text:'Sản phẩm & workflow library', view:'catalog' }
  ];
