...
| ~~`/trang-chu-v2`~~ | ~~Non-canonical Villa migration evidence; same public audience~~ | N/A — file deleted | **RETIRED — 07/08/2026 (L0 C-18)** | The "post-cutover disposition decision" this row was waiting on is now resolved: retirement, not further containment. | Route removed. `VillaPage.tsx` simplified to a single-route contract (no more `pageUrl` prop). No redirect was requested or added. |
...
1. **P1 — Canonical Villa Cutover — COMPLETED (PR #112 at 733b199):** `/` renders the canonical Villa through the shared `VillaPage.tsx`; shared home links corrected; `/trang-chu-v2` contained and noindex at the time. **Update 07/08/2026 (L0 C-18):** the post-cutover disposition this step deferred is now resolved — `/trang-chu-v2` is retired and removed, not merely contained.
