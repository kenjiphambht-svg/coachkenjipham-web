...
| Content authority | C-01, C-02, C-07, C-08; approved root Page Contract and copy. ~~`/trang-chu-v2` is implementation evidence only.~~ **07/08/2026 (C-19):** `/trang-chu-v2` is retired and removed — there is no second route left to hold implementation evidence. |
...
| Dependencies | Root Page Contract, exact metadata/copy, shared-header scope, asset approval, explicit noindex preservation, ~~post-cutover V2 containment decision~~ **(07/08/2026, C-19: resolved — retired, not contained; no longer an open dependency)**. |
| Non-goals | No V2 redirect/deletion, global design-system rewrite, offer activation, child/payment/form/backend work, sitemap, robots or indexing opening. *(P1-scoped; superseded for `/trang-chu-v2` specifically by the later, separately-approved C-19 retirement — see note below.)* |
| Definition of Done | `/` renders the approved Villa; exact C-07; all shared home links return to `/`; adult/parent doors work; no legacy CTA; keyboard/focus/contrast and desktop/mobile overflow checks pass; governed noindex remains; ~~`/trang-chu-v2` remains contained/noindex~~ **(07/08/2026, C-19: `/trang-chu-v2` is retired and removed, not contained — this P1 criterion no longer applies)**; production build and rendered link/meta QA pass. |
