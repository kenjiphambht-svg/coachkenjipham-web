...
| Production redirect currently contradicts C-14. | `https://coachkenjipham.com` → 307 → `https://www.coachkenjipham.com` (200). Source of the rule is the Vercel dashboard, not `vercel.json`/`next.config.mjs`. | Recorded as **O-07**. Must be corrected in the Vercel dashboard before `/` can be called domain-consistent. Not an indexing action and not fixable from the repository without creating a redirect loop. |
