...
| ~~No runtime canonical link policy was found.~~ **Superseded 02/08/2026:** the Villa routes now emit a canonical link. | `src/components/homepage/VillaPage.tsx` emits `<link rel="canonical">` from its `pageUrl` prop, verified in built HTML: `/` → `https://coachkenjipham.com/`, `/trang-chu-v2` → self-canonical. | **L0 C-14** fixes the canonical domain as the apex `https://coachkenjipham.com`. The tag is inert for indexing while both routes stay `noindex`; it exists so M6 needs no SEO retrofit. Canonical policy for **non-Villa routes** remains an M6 decision. |
