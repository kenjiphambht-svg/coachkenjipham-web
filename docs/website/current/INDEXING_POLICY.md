...
| ~~No runtime canonical link policy was found.~~ **Superseded 02/08/2026:** the Villa route now emits a canonical link. | `src/components/homepage/VillaPage.tsx` emits `<link rel="canonical">` from a fixed constant (07/08/2026 — was the `pageUrl` prop, self-canonical per route; `/trang-chu-v2` retired, L0 C-18, so the dual-route prop was removed), verified in built HTML: `/` → `https://coachkenjipham.com/`. | **L0 C-14** fixes the canonical domain as the apex `https://coachkenjipham.com`. The tag is inert for indexing while `/` stays `noindex`; it exists so M6 needs no SEO retrofit. Canonical policy for **non-Villa routes** remains an M6 decision. |
| Production host normalisation now matches C-14. | Verified live 02/08/2026: apex `https://coachkenjipham.com` serves 200 with 0 redirects; `www` returns 308 permanent to apex; HTTP redirects to HTTPS apex; no loop at any entry point; TLS valid on both hostnames. | **O-07 closed.** Host normalisation is a Vercel dashboard setting; `vercel.json`/`next.config.mjs` contain no host redirect and must not gain one. This is a domain-consistency fact, not an indexing action — C-02 `noindex` is unchanged. |
