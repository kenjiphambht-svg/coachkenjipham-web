...
| /lang-90/dat-phien | src/pages/lang-90/dat-phien.tsx; src/pages/api/lang-90/dang-ky.ts | Implemented six-question server intake | Private operation, not public sitemap | Observed noindex; governed noindex | POSTs validated intake; browser keeps only returned order code and routes to confirmation. | Human review source exists; migration/RLS integration verification remains required. |
| /lang-90/xac-nhan | src/pages/lang-90/xac-nhan.tsx | Implemented post-intake hold page | Private operation, not public sitemap | Observed noindex; governed noindex | No mailto, payment instruction or booking link before Kenji review. | L0 still requires support report, payment confirmation and private booking in later private steps. |
...
| /an-pham-ban-sac-hat-mam | src/pages/an-pham-ban-sac-hat-mam.tsx; src/components/landing-hat-mam/Room6Decision.tsx | Implemented landing; purchase flow hard-blocked | Hạt Mầm 0–7 discovery | Observed noindex; governed noindex pre-M6 | Package controls are disabled “Tạm chưa mở”; no external Tally URL remains. | **L0 C-06:** current Founder child-flow decision sheet is required before form/payment/confirmation/delivery/email work. |
...
| /lien-he | src/pages/lien-he.tsx; src/pages/api/lien-he.ts | Implemented server-backed contact form | Support node, not canonical product flow | Observed noindex; governed noindex pre-M6 | Form writes private inbox; still links partner asset, no mailto or automated email. | Staging verification and contact-operational reconciliation remain required. |
...
| /admin/* | src/pages/admin/*; middleware.ts; src/lib/auth/require-admin.ts | Implemented internal route family | Internal operations only | noindex on login; matcher blocks the family from unauthenticated/AAL1 sessions | Supabase session + AAL2 + active `admin_users` + RLS; no public journey role. | Staging MFA/RLS verification remains required; never add to sitemap. |
...
- No dynamic route file, robots file, sitemap file, Next.js rewrite, or sitemap implementation was found. Server API routes now exist for Lặng intake, contact intake and admin transition; they are not public sitemap nodes.
