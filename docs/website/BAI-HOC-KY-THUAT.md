...
## 12. KIẾN TRÚC LÀM SONG SONG NHIỀU TRANG / NHIỀU AGENT (chốt 24/07/2026)

**Sự cố 24/07:** một phiên (Claude Code) mở máy lên gặp thay đổi chưa commit
của phiên khác (Codex làm `/lang-90`) nằm sẵn trong CÙNG một thư mục repo —
vì cả 2 agent đang dùng chung 1 thư mục vật lý, chỉ khác nhau ở việc đổi qua
đổi lại branch (`git checkout`). Phải dừng lại `git stash` an toàn trước khi
tiếp tục (không mất gì, nhưng mất thời gian + rủi ro nếu agent không dừng lại
kiểm tra mà đổi thẳng branch).

Khi có ≥2 agent (Claude Code, Codex, hoặc agent khác) làm việc CÙNG LÚC trên
cùng 1 dự án web:

1. **MỘT website → MỘT repo chính** (`coachkenjipham-web`).
2. **MỖI trang đang làm → MỘT branch riêng.**
3. **MỖI agent đang chạy song song → MỘT WORKTREE riêng** (dùng lệnh
   `git worktree add <đường-dẫn> <branch>`) — mỗi agent có MỘT THƯ MỤC VẬT LÝ
   TÁCH BIỆT trên đĩa, KHÔNG dùng chung 1 thư mục rồi đổi qua đổi lại branch.
   Đây là nguyên nhân trực tiếp của sự cố 24/07: thay đổi chưa lưu của phiên
   này bị phiên khác vô tình chạm vào khi mở cùng thư mục. **Branch riêng
   KHÔNG đủ nếu vẫn chung thư mục** — worktree mới là giải pháp triệt để.
4. **MỖI trang trong code → component, dữ liệu, hình ảnh RIÊNG** — không dùng
   chung file trừ danh sách global tuyệt đối không đụng (`HomeHeader.tsx`,
   `HomeFooter.tsx`, `globals.css`, `tailwind.config.ts`).

**QUY TẮC:** trước khi bắt đầu bất kỳ session nào mà biết trước sẽ có agent
khác chạy song song, TỰ KIỂM xem repo đang ở dạng thư mục đơn (dễ đụng) hay
đã dùng worktree — nếu chưa dùng worktree và có dấu hiệu agent khác đang hoạt
