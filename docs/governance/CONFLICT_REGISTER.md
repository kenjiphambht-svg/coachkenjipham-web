...
| C-17 | Product activation versus indexing | A product may be configured/tested or later activated only through its own contract; this never removes noindex or adds sitemap/indexing. Older checklist implications are superseded: M6 remains the only indexing decision. |
| C-18 | ESSENCE Language, Method & Journey Rhythm (06/08/2026, corrected 07/08/2026) | ESSENCE connects existing concepts into an **additional orientation map** — standing alongside, not replacing, the existing three-movement ESSENCE method framework (**Chill với cảm xúc → Thách thức giới hạn → Hiện thực ước mơ**, unchanged canonical brand/method asset; no forced 1:1 mapping between the two layers without separate Founder approval; the future `/phuong-phap` Page Contract must explain both layers): **LẶNG → AN ĐỊNH → NHẬN RA NHỊP RIÊNG → NHÌN RA BẢN SẮC → CHỌN MỘT NHỊP SỐNG CÓ THỂ GIỮ → AN THỊNH** (“Một bản đồ để trở về đúng nhịp”). It is non-linear, non-diagnostic, has no tests/scoring/result guarantees. Terms (Nhịp, Nhịp riêng, Nhịp sống, Lệch nhịp, Trở về đúng nhịp) are life concepts, not psychological profiles. Lặng and Hạt Mầm are independent doors, not a forced upsell funnel. Frontend explains framework cleanly; backend/Care OS uses journey rhythm for care orchestration (events, consent, suppression, Human Decision Gate), not DB fields or scores. AI must not infer true identity/rhythm, dán nhãn, or use child data for sales. Full correction: `docs/decisions/FD-2026-08-06_ESSENCE_LANGUAGE_METHOD_AND_JOURNEY_RHYTHM.md` §1.1. |
