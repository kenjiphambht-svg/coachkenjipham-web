...
| C-03 | Legacy child/payment flow | ~~/kidbook, /thanh-toan-goi-1 and /thanh-toan-goi-2 are excluded from new journey, CTA, migration and payment flows.~~ **Superseded by C-17 (02/08/2026):** indefinite containment is retired; all three routes are LEGACY RETIRED — one-pass redirect, not migration/replacement. |
| C-04 | /ai-startup role | noindex; independent partner asset; outside consumer journey; rewrite pending. |
| C-05 | Lặng capacity/flow | 5 sessions/month; six questions → support report → Kenji Human Decision Gate → payment → Kenji payment confirmation → private booking link. **Clarification (FD-B, 02/08/2026):** this sequence has two distinct gates, not one. Gate 1, Human Decision Gate — Kenji manually reads and decides accept/decline/ask-more; never automated, never AI-scored. Gate 2, Payment Confirmation — an accounting fact (funds arrived), not a judgment; a bank-signal-triggered automated state change that fires the confirmation email is compliant with this ruling and does not weaken Gate 1. The 5/month capacity must lock at the **payment-link-issuance step**, not the final booking step — a client paying and only then learning the month is full is a serious defect, not an acceptable edge case. |
...
| C-17 | Legacy route retirement (FD-2026-08-02, FD-A) | Founder confirmed `/kidbook` was demo-only, with no live or pending orders on any legacy route — so retirement happens in one pass, not a phased migration. `/kidbook`, `/thanh-toan-goi-1` and `/thanh-toan-goi-2` are **LEGACY RETIRED — APPROVED**: each returns a permanent (301) redirect to `/an-pham-ban-sac-hat-mam`; source files are archived under `archive/legacy-routes/` (out of `src/pages`, kept in git history — not deleted). Precondition before merge: confirm the two Tally forms (`tally.so/r/1ANjJ4`, `tally.so/r/Y5J2VN`) do not have "redirect on completion" pointed at either retired payment route; if they do, repoint Tally first. The 9 legacy CSS variables in `globals.css` and the `useMistFadeIn` hook are unaffected by this retirement (still used by `/ai-startup`, and globally via `MistFadeProvider` in `_app.tsx`, respectively) and must not be removed as a side effect. |
