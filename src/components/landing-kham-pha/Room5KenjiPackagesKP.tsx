...
import { LINE_STATUS, KHAM_PHA_PRICING } from "@/config/khamPhaLaunch";

// ============================================================
// PHÒNG 6 — KENJI & GÓI. Tái dùng pattern "S10 Kenji là ai" (portrait +
// bio, Room5KenjiProcess.tsx) + "S12 hai gói" (Room6Decision.tsx) của
// Hạt Mầm. Copy NGUYÊN VĂN theo task.
//
// ⚠️ ĐOẠN KENJI CHƯA CÓ: task ghi "[KENJI VIẾT — 3-4 câu quan sát 10
// năm về tuổi 7-14...]" — Kenji CHƯA điền đoạn này khi gửi task (phần
// dành để dán SAU KHI điền vẫn còn nguyên placeholder). Không tự bịa
// giọng Kenji — để khối placeholder rõ ràng, chờ Kenji cung cấp.
//
// LINE_STATUS (src/config/khamPhaLaunch.ts) điều khiển khối giá + CTA
// gói mà KHÔNG cần sửa layout: 'preview' → ẩn giá, CTA = nút viền
// "Ghi danh sớm" (mailto, cùng pattern /ban-la-duy-nhat, /dau-an-cua-ban);
// 'live' → hiện giá thật + nút vàng dẫn form đặt thật.
// ============================================================
export default function Room5KenjiPackagesKP() {
  ...
  const handleRegister = (e: FormEvent) => {
    ...
    if (!isBot) {
      const subject = "Ghi danh sớm — Bản Sắc Khám Phá";
            ...
            <div className="md:col-span-8 max-w-[620px]">
              ...
              <div className="hm-reveal border border-e26-border bg-e26-white p-6 mb-8">
                <p className="font-sans text-[15px] leading-[1.7] text-e26-text-2">
                  [KENJI VIẾT — chờ 3-4 câu quan sát 10 năm về tuổi 7-14: điều gì anh hay thấy lặp
                  lại ở ba mẹ có con tuổi này, điều gì anh ước ba mẹ biết sớm hơn. Chưa có nội
                  dung — không tự soạn thay Kenji.]
                </p>
              </div>
          ...
          <div className="grid md:grid-cols-2 gap-10 md:gap-8 mb-14">
            ...
            <div className="hm-reveal bg-e26-white border border-e26-border p-8 md:p-10 flex flex-col">
              ...
              {isPreview ? (
                ...
                <a
                  href={KHAM_PHA_PRICING.goi1.formUrl}
            ...
            <div className="hm-reveal hm-d1 bg-e26-white border border-e26-border p-8 md:p-10 flex flex-col">
              <h3 className="font-serif text-2xl text-e26-text mb-4">
                {KHAM_PHA_PRICING.goi2.name}
              </h3>
              <p className="font-sans text-[15px] leading-[1.65] text-e26-text-2 mb-6 flex-1">
                Ấn phẩm + 45 phút trò chuyện riêng về con.
              </p>
              <p className="font-serif text-2xl text-e26-text mb-6">
                {isPreview ? "Giá công bố khi mở cửa" : KHAM_PHA_PRICING.goi2.priceDisplay}
              </p>
              {isPreview ? (
                <a
                  href="#ghi-danh"
                ...
                <a
                  href={KHAM_PHA_PRICING.goi2.formUrl}
                  className="mt-auto inline-block text-center bg-e26-gold text-e26-black rounded-none font-sans font-medium text-[13px] tracking-[0.08em] uppercase px-9 py-4 hover:bg-e26-gold-deep hover:text-e26-ivory transition-colors duration-300"
                >
