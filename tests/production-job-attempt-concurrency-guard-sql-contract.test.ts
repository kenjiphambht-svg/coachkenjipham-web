...
const rollback = readFileSync(
  resolve(process.cwd(), 'supabase/rollbacks/20260814035042_launch_core_production_job_attempt_concurrency_guard_down.sql'),
  'utf8'
);
...
describe('WO-LAUNCH-CORE-04 self-review — Job Attempt concurrency guard', () => {
  it('is additive-only: no new table, only a partial unique index', () => {
    expect(code).not.toMatch(/create table|drop table|create schema|drop schema|create trigger|create or replace function/i);
  });

  it('adds an atomic partial unique index limiting a Job to one running Attempt at a time', () => {
    expect(sql).toMatch(
      /create unique index job_attempts_one_running_per_job_idx\s*on production\.job_attempts\(job_id\)\s*where status = 'running'/i
    );
  });
});
...
describe('scoped rollback', () => {
  it('drops only the new index', () => {
    expect(rollback).toMatch(/drop index if exists production\.job_attempts_one_running_per_job_idx/i);
